...
### Format: `T-XXX-E<Number>-<Attempt>`
- **`T-XXX` (Task ID)**: รหัสงานหลักที่ปัญหานั้นเกิดขึ้น
- **`E<Number>` (Error Index)**: ลำดับของ "ปัญหา" (Problem) ที่พบ (รันเลข E1, E2, ... ตามลำดับ)
  - *Rule*: หากเป็นปัญหาเรื่องเดิม อาการเดิม หรือ Root Cause เดิม ให้ใช้เลข **E เดิม** เสมอ แม้จะแก้หลายรอบ
- **`<Attempt>` (Attempt Count)**: จำนวนครั้งที่พยายามแก้ไข (เริ่มที่ 1)
  - หากแก้ครั้งแรกไม่หาย แล้วต้องแก้ซ้ำ ให้เพิ่มเลขนี้เป็น 2, 3, ... (เช่น E1-2, E1-3)
  - **ห้าม** เปลี่ยนเลข E ถ้ายังเป็นปัญหาเดิม

### Examples & Anti-Patterns:
- ✅ **Correct**: `[T-003-E1-1]` -> (Error ตัวที่ 1, แก้ครั้งที่ 1)
- ✅ **Correct**: `[T-003-E1-2]` -> (Error ตัวที่ 1, แก้ครั้งที่ 2 - ยังไม่หาย)
- ✅ **Correct**: `[T-003-E2-1]` -> (Error ตัวที่ 2, เป็นปัญหาใหม่คนละเรื่องกับ E1)

- ❌ **Incorrect**: `[T-003-EX-1]` -> (ห้ามใช้ EX เป็นตัวอักษร)
- ❌ **Incorrect**: `[T-003-E1-1]`, `[T-003-E2-1]` -> (ผิดบริบท: ถ้าแก้เรื่องเดิม ต้องใช้ E1-2 ไม่ใช่เปิด E2 ใหม่)
...
- [ ] [T-XXX] Implement Feature A
    - **Error Logs**:
      - **[T-XXX-E1-1]**: Import Error
        ...
        3. **Status**: Fixed
      - **[T-XXX-E2-1]**: Data Missing
        1. **Root Cause**: ลืมใส่ await
        2. **Action**: ใส่ await
        3. **Status**: Fixed
```
