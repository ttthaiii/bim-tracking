...
const ProjectsPage = () => {
  ...
  // ========== ✨ เพิ่มใหม่: useEffect คำนวณจำนวนงานที่ต้อง Rev. ==========
useEffect(() => {
  if (!cacheLoaded || allTasksCache.length === 0) {
    return;
  }
  ...
  const needsRevision = tasksToCheck.filter(t => {
    ...
    // ⚠️ เช็คใน tasksToCheck (ของโครงการนี้) ไม่ใช่ allTasksCache
...
}, [allTasksCache, cacheLoaded, selectedProject]); // ← เพิ่ม selectedProject
...
const handleEdit = (idx: number) => {
  ...
  // ========== ✨ ฟังก์ชันสำหรับเลือกงานที่จะสร้าง Revision ==========
  const handleSelectTaskForRevision = (task: any) => {
    // ค้นหา tasks ที่เกี่ยวข้อง (รวม REV.)
    ...
    // ดึง Document Number จากแถวต้นฉบับ
